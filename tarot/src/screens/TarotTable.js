...
class TarotTable extends Component {
  ...
  headMessage() {
    ...
    const message = `
      ...
      张塔罗牌
    `;
    return <h1 className="page-title">请选择{message}</h1>;
  }
    ...
    for (let i = 0; i < amountCards; i++) {
      cards.push(
        <Col lg="2" md="2" sm="2" xs="3" key={i}>
          <CardContainer cardNumber={i} reversed={this.reversedCard()} handleChoice={choice => this.state.canChoice && this.props.chooseCard(choice)} />
        </Col>
      );
    }
