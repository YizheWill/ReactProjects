...
const Reading = ({ userChoiceState, spreadTypeState, navigate }) => {
  ...
  const cardInfo = userChoiceState.map((card, i) => {
    return <CardDetailsContainer key={i} cardNumber={card} choices={userChoiceState} spreadType={spreadTypeState} />;
  });
  ...
  return (
    <div className="reading">
      <RaisedButton label="抽牌" style={btnStyle} onTouchTap={() => navigate("/")} />
      <RaisedButton label="记录" style={btnStyle} onTouchTap={() => navigate("/save-game")} />
      {cardInfo}
    </div>
  );
};
